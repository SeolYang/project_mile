...
namespace Mile
{
   ...
   bool LightBufferPass::Bind(ID3D11DeviceContext& deviceContext)
   {
      ...
      if (!m_lightParamBuffer->Bind(deviceContext, 0, EShaderType::PixelShader))
      {
         return false;
      }

      if (!m_cameraBuffer->Bind(deviceContext, 1, EShaderType::PixelShader))
      {
         return false;
      }

      if (!m_checkerBoardBuffer->Bind(deviceContext, 2, EShaderType::PixelShader))
      {
         return false;
      }
   ...
   void LightBufferPass::UpdateLightParamBuffer(ID3D11DeviceContext& deviceContext, 
